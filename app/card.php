...
<?= cardActionNotice() ?>
<?php if(!$userId): ?><section class="card-actions panel" data-card-actions><div><h2><?= te('Usar esta carta') ?></h2><p><?= te('Entre na sua conta para adicioná-la a um deck ou abrir um planejamento com ela.') ?></p></div><a class="primary-link" href="/login.php?next=<?= h(rawurlencode('/card.php?id='.$card['id'])) ?>"><?= te('Entrar') ?></a></section><?php else: ?><section class="card-actions panel" data-card-actions><div><h2><?= te('Usar esta carta') ?></h2><p><?= te('Guarde esta impressão na sua coleção ou mande a carta para as candidatas de um deck que aceite as cores dela.') ?></p></div><?php cardActionsMenu($card, cardActionDecks($userId), $userId, '/card.php?id='.$card['id'], wishlistLogicalIds($userId)); ?><?php if($isCommander): ?><form method="post" class="card-action-form commander-action"><input type="hidden" name="csrf" value="<?= h($builderCsrf) ?>"><input type="hidden" name="action" value="create_commander_deck"><label><?= te('Nome do novo deck') ?><input name="name" value="<?= h($card['name'].' — planejamento') ?>" maxlength="160"></label><button class="secondary-link"><?= te('Abrir deck com esta comandante') ?></button></form><?php endif; ?></section><?php endif; ?>
<div class="detail">
