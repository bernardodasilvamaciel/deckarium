...
function cardFilterForm(array $f,array $sets,string $action,string $view=''): void {
    ...
    <label class="field"><?= te('Tipos e temas') ?><input name="type" value="<?= h($f['type']) ?>" placeholder="pirate; assassin; vehicle; treasure"><small>Qualquer termo no tipo ou Oracle.</small></label>
    ...
    <label class="field"><?= te('Custo de mana') ?><input name="mana" value="<?= h($f['mana']) ?>" placeholder="{2}{G}{G}"><small>Sequência de símbolos no custo.</small></label>
    ...
    </div><p class="muted">Oracle: todos os termos separados por ponto e vírgula. Poder, resistência e lealdade: apenas valores numéricos.</p><button><?= te('Aplicar filtros') ?></button></details><a href="<?= h($action) ?>"><?= te('Limpar filtros') ?></a></form><?php
    filterPanelEnd();
}
