...
<label><input type="checkbox" name="synergy" value="1" <?= $showSynergy?'checked':'' ?>> Sinergia do comandante</label><label><input type="checkbox" name="outside" value="1" <?= $includeOutside?'checked':'' ?>> Incluir cartas fora da coleção</label><button class="secondary-link">Aplicar filtros</button><span><?= $synergyCount ?> cartas · maior sinergia primeiro</span></form>
