...
const base=process.env.DECKARIUM_BASE||'http://localhost:8080'; let cookie=await login(base); const printing=randomUUID();
...
const csv=`Name,Scryfall ID,Quantity,Foil\nVerificação temporária,${printing},2,normal\nVerificação temporária,${printing},1,foil\n`;
...
response=await request('/collection.php',{method:'POST',body:new URLSearchParams({csrf:token(html),action:'delete',card:printing,foil:'0'})});assert.equal(response.status,303,await response.text());
html=await page('/collection.php');assert(html.includes('Verificação temporária removida'));
response=await request('/collection.php',{method:'POST',body:new URLSearchParams({csrf:token(html),action:'delete',card:printing,foil:'1'})});assert.equal(response.status,303,await response.text());
html=await page('/collection.php');assert(html.includes('foil de Verificação temporária removida'));
console.log('PASS: importação CSV incremental separa acabamento normal e foil e permite excluir cada versão.');
